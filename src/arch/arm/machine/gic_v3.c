...
get_mpidr(word_t core_id)
{
    return mpidr_map[core_id];
}

static inline uint64_t
get_current_mpidr(void)
{
...
gicr_locate_interface(void)
{
    ...
    for (offset = 0; offset < GICR_SIZE; offset += GICR_PER_CORE_SIZE) {
        ...
        if ((typer >> 32) == ((MPIDR_AFF3(mpidr) << 24) |
            ...
            assert(!(val & GICR_WAKER_ChildrenAsleep));
...
gicr_init(void)
{
    ...
    gic_rdist_sgi_ppi_map[CURRENT_CPU_INDEX()]->icfgr1 = 0xaaaaaaaa;
...
void setIRQTrigger(irq_t irq, bool_t trigger)
{
    ...
    /* GIC-500 has read-only GICR_ICFG0 for SGI with
    ...
    if (is_ppi(irq)) {
         ...
         * be disabled before chaning the field, and this function
